...
class Episode:
    ...
    def terminate(self):
        """Packs the episode timesteps into arrays"""
        s = jnp.stack(self.s)
        a = jnp.stack(self.a)
        r = jnp.stack(self.r)
        d = jnp.stack(self.d)
        return Episode(s, a, r, d)
    ...
    def add(
        ...
        self.s.append(jnp.asarray(timestep.observation, dtype=jnp.float32))
        self.a.append(action)
        self.r.append(jnp.asarray([timestep.reward], dtype=jnp.float32))
        ...
        self.d.append(jnp.array([timestep.last() * int(truncated)], dtype=jnp.int32))
        return
